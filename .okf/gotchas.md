...
- Durable startup/offline remote events require matching known subscriber IDs on both sides: `c.SubscriberID = SubscriberService.Name` before subscriber `c.Subscribe<...>()` calls and `RegisterEventHub<TEvent>([SubscriberService.Name])` in the publisher.
