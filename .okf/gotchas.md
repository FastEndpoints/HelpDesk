...
- Notifications job idempotency lasts while the `JobRecord` row exists (including completed). After purge/delete the same key may enqueue again. Null/empty/whitespace keys are not deduped. Duplicate verification-issued events share `UserIdentityVerificationIssuedEventHandler.JobIdempotencyKey(UserIdentityId, VerificationCode)` (`user-identity-verification:{id}:{code}`) and return the existing tracking id without a second row. Resend rotates the code so a new key/job is created. Password-reset emails use `user-identity-password-reset:{id}:{code}` the same way.
...
- Notifications personalizes emails from a local `DisplayNames` projection (Profile registered + display-name-updated events), not by calling Profile. Missing projection falls back to email local-part. Projection upserts ignore older `UpdatedAt` values so out-of-order events do not clobber newer names.
...
- Verification codes have no expiry and are not cleared after activation; do not assume one-time or time-limited links. Resend replaces the stored code for Deactivated identities only after 30 minutes since `VerificationIssuedAt` (register sets it; resend advances it). Old email links stop matching once replaced. Cooldown no-ops still return the generic success string.
- Password reset is the opposite model: one-shot hashed tokens, 30m lifetime, 30m re-request cooldown, deleted on use. Do not reuse verification assumptions for reset. Forgot-password always returns `Reset-Available-In` (unlike opaque resend-verification); Active remaining under full 30m can imply a recent reset request.
- Verification and password-reset links require `UserIdentity:FrontendBaseUrl`. Production must set explicit Identity frontend and Profile picture public bases because backend services do not configure forwarded-header middleware.
