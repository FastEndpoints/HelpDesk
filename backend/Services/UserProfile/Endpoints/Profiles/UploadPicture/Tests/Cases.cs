...
{
    [Fact]
    public async Task OpenApi_Describes_File_As_Binary()
    {
        using var client = App.CreateClient();
        using var response = await client.GetAsync("/openapi/v1.json", Cancellation);
        response.EnsureSuccessStatusCode();

        var json = await response.Content.ReadAsStringAsync(Cancellation);
        using var document = JsonDocument.Parse(json);
        var fileSchema = document.RootElement
                                 .GetProperty("components")
                                 .GetProperty("schemas")
                                 .GetProperty("EndpointsProfilesUploadPictureRequest")
                                 .GetProperty("properties")
                                 .GetProperty("file");

        fileSchema.GetProperty("type").GetString().ShouldBe("string");
        fileSchema.GetProperty("format").GetString().ShouldBe("binary");
        fileSchema.TryGetProperty("$ref", out _).ShouldBeFalse();
        json.ShouldNotContain("/IFormFile");

        document.RootElement
                .GetProperty("paths")
                .GetProperty("/profiles/me/picture")
                .GetProperty("put")
